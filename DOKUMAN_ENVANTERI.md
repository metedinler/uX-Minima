# Dokuman Envanteri (Calisma Klasoru + PCK)

## Operasyonel Ana Belge

- MASTER_TAKIP_DOKUMANI_V31.md

Not:
Bu envanterdeki tum kaynaklarin guncel takip ve karar ozetleri ana belgeye islenir.

## Calisma Klasorunde Tespit Edilen Ana MD/TXT

- uX_Minima_Kullanim Klavuzu.md
- ux_minima_full v31.md
- ux_minima_IDE_vscode.md
- ux_minimax_ide_tasarimi_dusuncesi.md
- gercek_durum.md
- yeni tasarim eksiklikler.md
- final/UXM31_FINAL_ARGE_COMPILER.md
- ide/uxminima-vscode/docs/*.md
- ide/uxminima-vscode/README.md
- ide/uxminima-vscode/ide_okubeni.md
- math_extensions/docs/UXM_MATH_EXTENSIONS.md
- tests/UX-MINIMA x64 V3.1 Tasarim Belgesi.md
- UX-FP V1/Decimal_FP_Sistemi.md
- FILE_MANIFEST.txt

## Calisma Klasoru Disi PCK Belgesi

- ../../uxm_x64/uxm_x64 v1/pck.md

## Not
Bu envanter, matris ve gap raporlarinin kaynak listesidir.
