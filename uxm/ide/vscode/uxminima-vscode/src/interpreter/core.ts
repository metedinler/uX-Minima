...
import { stripDefinitionsText } from "./parser";
import { buildUnsupportedMetaMessage } from "./metaRuntime";
import { collectNonZero, collectWindow } from "./trace";
import { cellMaskForBits, scaleForBits, signBitForBits } from "./memory";
...
export class UxmInterpreter {
  ...
  private stripDefinitions(source: string): string {
    return stripDefinitionsText(source);
  }
  ...
  private meta(id: number, forceHost = false): void {
    ...
    switch (id) {
      ...
      case 150: this.flags &= ~0x20; this.setStatus(0); break;
      case 151: this.flags |= 0x20; this.setStatus(0); break;
      case 152: result = (this.flags & 0x20) ? 1 : 0; break;
      case 153:
      ...
      case 160:
        if (!this.matInit(this.readTape(this.ptr - 4), this.readTape(this.ptr - 3), this.readTape(this.ptr - 2), this.readTape(this.ptr - 1), this.readTape(this.ptr))) this.setStatus(16); else this.setStatus(0);
        result = this.status;
        break;
      case 161:
        if (!this.matValid(this.readTape(this.ptr - 4))) this.setStatus(16); else {
          const base = this.readTape(this.ptr - 4);
          const n = this.data[base + 10] | 0;
          for (let i = 0; i < n; i++) this.data[base + 16 + i] = 0;
          this.setStatus(0);
        }
        result = this.status;
        break;
      case 162: {
        ...
        if (idx < 0) this.setStatus(16); else { this.data[idx] = val & this.mask(); this.setStatus(0); }
        result = this.status;
      } break;
      case 163: {
      ...
      case 164:
        if (!this.matValid(this.readTape(this.ptr - 4))) this.setStatus(16); else {
          ...
          for (let i = 0; i < n; i++) this.data[base + 16 + i] = v;
          this.setStatus(0);
        }
        result = this.status;
        break;
      case 165:
        if (!this.matValid(this.readTape(this.ptr - 4)) || !this.matValid(this.readTape(this.ptr - 3))) this.setStatus(16); else {
          const dst = this.readTape(this.ptr - 4);
          const src = this.readTape(this.ptr - 3);
          const n = this.data[src + 10] | 0;
          if ((this.data[dst + 10] | 0) !== n) this.setStatus(16); else {
            for (let i = 0; i < n; i++) this.data[dst + 16 + i] = this.data[src + 16 + i] ?? 0;
            this.setStatus(0);
          }
        }
        result = this.status;
        break;
      case 166:
        if (!this.matValid(this.readTape(this.ptr - 3))) this.setStatus(16); else {
      ...
      case 167:
      case 168:
        if (!this.matValid(this.readTape(this.ptr - 4)) || !this.matValid(this.readTape(this.ptr - 3)) || !this.matValid(this.readTape(this.ptr - 2))) this.setStatus(16); else {
          const dst = this.readTape(this.ptr - 4);
          const aBase = this.readTape(this.ptr - 3);
          const bBase = this.readTape(this.ptr - 2);
          const n = this.data[dst + 10] | 0;
          if ((this.data[aBase + 10] | 0) !== n || (this.data[bBase + 10] | 0) !== n) this.setStatus(16); else {
            for (let i = 0; i < n; i++) {
      ...
      case 169:
        if (!this.matValid(this.readTape(this.ptr - 4)) || !this.matValid(this.readTape(this.ptr - 3))) this.setStatus(16); else {
          ...
          const n = this.data[dst + 10] | 0;
          if ((this.data[src + 10] | 0) !== n) this.setStatus(16); else {
            const s = this.toSignedCell(arg0);
            for (let i = 0; i < n; i++) this.data[dst + 16 + i] = (this.toSignedCell(this.data[src + 16 + i] ?? 0) * this.toSignedCell(scalar)) & this.mask();
            this.setStatus(0);
          }
        }
        result = this.status;
      ...
      case 170:
        if (!this.matValid(this.readTape(this.ptr - 4)) || !this.matValid(this.readTape(this.ptr - 3)) || !this.matValid(this.readTape(this.ptr - 2))) this.setStatus(16); else {
          ...
          const aBase = this.readTape(this.ptr - 3);
          const bBase = this.readTape(this.ptr - 2);
          const ar = this.data[aBase + 5] | 0; const ac = this.data[aBase + 6] | 0;
          const br = this.data[bBase + 5] | 0; const bc = this.data[bBase + 6] | 0;
          if (ac !== br || (this.data[dst + 5] | 0) !== ar || (this.data[dst + 6] | 0) !== bc) this.setStatus(16); else {
      ...
      case 171:
        if (!this.matValid(this.readTape(this.ptr - 4)) || !this.matValid(this.readTape(this.ptr - 3))) this.setStatus(16); else {
          const dst = this.readTape(this.ptr - 4);
          const src = this.readTape(this.ptr - 3);
          const rs = this.data[src + 5] | 0; const cs = this.data[src + 6] | 0;
          if ((this.data[dst + 5] | 0) !== cs || (this.data[dst + 6] | 0) !== rs) this.setStatus(16); else {
        ...
        break;
      case 172:
        if (!this.matInit(this.readTape(this.ptr - 4), this.readTape(this.ptr - 3), this.readTape(this.ptr - 3), this.readTape(this.ptr - 2), this.readTape(this.ptr - 1))) this.setStatus(16); else {
          const base = this.readTape(this.ptr - 4);
          ...
          for (let i = 0; i < n; i++) {
            const idx = this.matCellIndex(base, i, i);
            if (idx >= 0) this.data[idx] = one & this.mask();
          }
          this.setStatus(0);
        }
        result = this.status;
        break;
      case 173:
        if (!this.matValid(this.readTape(this.ptr - 3)) || (this.data[this.readTape(this.ptr - 3) + 5] | 0) !== (this.data[this.readTape(this.ptr - 3) + 6] | 0)) this.setStatus(16); else {
          const base = this.readTape(this.ptr - 3);
      ...
      case 174:
        ...
        break;
      case 175:
        if (!this.matValid(this.readTape(this.ptr - 3)) || (this.data[this.readTape(this.ptr - 3) + 5] | 0) !== 2 || (this.data[this.readTape(this.ptr - 3) + 6] | 0) !== 2) this.setStatus(16); else {
          const base = this.readTape(this.ptr - 3);
      ...
      case 176:
        if (!this.matValid(this.readTape(this.ptr - 3))) this.setStatus(16); else {
          ...
          const n = this.data[base + 10] | 0;
          let s = "";
          for (let i = 0; i < n; i++) { if (i) s += ","; s += String(this.toSignedCell(this.data[base + 16 + i] ?? 0)); }
          this.output += s;
          this.setStatus(0);
        }
        result = this.status;
      ...
      case 252: {
        ...
        if (n <= 0) { this.setStatus(15); result = 0; break; }
        const h = (b - a) / n;
        let sum = 0;
        for (let i = 0; i <= n; i++) {
          const x = a + i * h;
          if (i === 0 || i === n) sum += this.exprEvalRpn(exprBase, Math.trunc(x));
          else sum += 2 * this.exprEvalRpn(exprBase, Math.trunc(x));
        }
      ...
      case 253: {
        ...
        const b = this.toSignedCell(this.readTape(this.ptr - 2));
        const n = this.toSignedCell(this.readTape(this.ptr - 1));
        if (n <= 0 || (n % 2) !== 0) { this.setStatus(5); result = 0; break; }
        const h = (b - a) / n;
        let sum = this.exprEvalRpn(exprBase, a) + this.exprEvalRpn(exprBase, b);
        for (let i = 1; i < n; i++) {
          const x = a + i * h;
          if ((i % 2) === 0) sum += 2 * this.exprEvalRpn(exprBase, Math.trunc(x));
          else sum += 4 * this.exprEvalRpn(exprBase, Math.trunc(x));
        }
  ...
  private markUnsupportedMeta(id: number, reason: string): void {
    ...
    this.diagnostics.push(buildUnsupportedMetaMessage(id, reason));
  ...
  private window(mem: number[], start: number, count: number): CellEntry[] {
    return collectWindow(mem, start, count, ascii);
  }

  private nonZero(mem: number[], limit: number): CellEntry[] {
    return collectNonZero(mem, limit, ascii);
  }
  ...
  private mask(): number { return cellMaskForBits(this.cellBits); }
  ...
  private signBit(): number { return signBitForBits(this.cellBits); }
  private scale(): number { return scaleForBits(this.cellBits); }
