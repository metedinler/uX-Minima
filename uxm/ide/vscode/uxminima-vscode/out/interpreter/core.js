...
const metaRuntime_1 = require("./metaRuntime");
const trace_1 = require("./trace");
const memory_1 = require("./memory");
class UxmInterpreter {
    constructor() {
        this.diagnostics = [];
        this.unsupportedMetaReported = new Set();
        this.instr = [];
    ...
    stripDefinitions(source) {
        return (0, parser_1.stripDefinitionsText)(source);
    }
    ...
    matrixEmitHeader(base, rows, cols, typ, scale) {
    ...
    parseDataPragma(rawLine) {
        ...
        if (lower.startsWith("#poly")) {
            ...
            if (!Number.isFinite(base) || vals.length === 0) {
                return;
            }
        ...
        if (cmd === "#matrix" || cmd === "#matrix-signed" || cmd === "#matrix-fixed") {
            ...
            if (!Number.isFinite(base) || !Number.isFinite(rows) || !Number.isFinite(cols) || rows <= 0 || cols <= 0) {
                return;
            }
            if (cmd === "#matrix-fixed") {
                ...
                for (let i = 0; i < need; i++)
                    this.addDataInit(base + 16 + i, this.fixedToScaled(vals[i], scale));
            }
            else {
                ...
                const need = Math.min(vals.length, rows * cols);
                for (let i = 0; i < need; i++)
                    this.addDataInit(base + 16 + i, vals[i]);
            }
            return;
        }
        if (cmd === "#identity") {
            ...
            if (!Number.isFinite(base) || !Number.isFinite(n) || n <= 0) {
                return;
            }
            this.matrixEmitHeader(base, n, n, 0, 0);
            for (let i = 0; i < n; i++)
                this.addDataInit(base + 16 + i * n + i, 1);
            return;
        }
    ...
    executeOne(ip, ins) {
        switch (ins.op) {
            ...
            case "META":
                this.meta(ins.metaDyn ? this.tape[this.ptr] : (ins.metaId ?? 0), ins.metaForceHost === true);
                return ip + 1;
    ...
    meta(id, forceHost = false) {
        ...
        switch (id) {
            ...
            case 2:
                ...
                break;
            case 3:
                result = Math.floor(this.rngNext() * 256);
                break;
            case 4:
                result = Date.now();
                break;
            ...
            case 9:
                result = this.status;
                preserveStatusAfterWrite = true;
                break;
            case 10:
                this.setStatus(0);
                break;
            case 11:
                this.setStatus(arg1 & 0xff);
                break;
            case 12:
                ...
                break;
            case 13:
                this.setStatus(this.status === 0 ? 1 : this.status);
                break;
            case 14:
                this.setStatus(0);
                break;
            case 15:
                result = (this.flags & 0x400) ? 1 : 0;
                preserveStatusAfterWrite = true;
                break;
            case 20:
                result = arg1 + arg2;
            ...
            case 27:
                {
                    if ((this.flags & 0x10) !== 0) {
                        let signed = arg2;
                        if (this.cellBits === 8 && (arg2 & 0x80))
                            signed = arg2 - 256;
                        else if (this.cellBits === 16 && (arg2 & 0x8000))
                            signed = arg2 - 65536;
                        else if (this.cellBits === 32 && (arg2 & 0x80000000))
                            signed = arg2 - 4294967296;
                ...
                break;
            case 28:
                result = (((~arg2) + 1) >>> 0) & this.mask();
                break;
            case 29:
                result = arg1 === arg2 ? 0 : (arg1 > arg2 ? 1 : this.mask());
                break;
            case 30:
                { // RND_RANGE: min=T-2, max=T-1 -> T+1
                    if (arg2 < arg1) {
                        result = arg1;
                    }
                    else {
                        result = arg1 + Math.floor(this.rngNext() * (arg2 - arg1 + 1));
                    }
                }
                break;
            ...
            case 34:
                { // DIV_SIGNED
                    ...
                    const toSigned = (v) => {
                        if (this.cellBits === 8 && (v & 0x80))
                            return v - 256;
                        if (this.cellBits === 16 && (v & 0x8000))
                            return v - 65536;
                        if (this.cellBits === 32 && (v & 0x80000000))
                            return v - 4294967296;
                        return v;
                    };
            ...
            case 43:
                ...
                break;
            case 44:
                result = Math.round(Math.asin(arg2 / this.scale()) * 180 / Math.PI);
                break;
            case 45:
                result = Math.round(Math.acos(arg2 / this.scale()) * 180 / Math.PI);
                break;
            case 46:
                result = Math.round(Math.sqrt(arg2));
                break;
            case 47:
                result = Math.round(Math.sinh(arg2 * Math.PI / 180) * this.scale());
                break;
            case 48:
                result = Math.round(Math.cosh(arg2 * Math.PI / 180) * this.scale());
                break;
            case 49:
                result = Math.round(Math.tanh(arg2 * Math.PI / 180) * this.scale());
                break;
            case 52:
                {
            ...
            case 64:
                ...
                break;
            case 67:
                this.output += (arg2 & this.mask()).toString(16).toUpperCase();
                this.setStatus(0);
                break;
            case 68:
                this.output += (arg2 & this.mask()).toString(2);
                this.setStatus(0);
                break;
            case 69:
                this.output += String.fromCharCode(arg2 & 0xff);
                this.setStatus(0);
                break;
            case 80:
                this.ptr = arg2;
            ...
            case 82:
                ...
                break;
            case 83:
                result = this.ptr < this.tape.length ? 1 : 0;
                break;
            case 84:
            ...
            case 86:
                ...
                break;
            case 87:
                result = this.cellBits;
                break;
            case 88:
                result = this.cellBits === 8 ? 1 : this.cellBits === 16 ? 2 : 4;
                break;
            case 89:
            ...
            case 201:
                ...
                if (arg1 + 1 < this.data.length)
                    this.data[arg1 + 1] = id === 201 ? 32 : 16;
                this.setStatus(0);
                break;
            case 202:
                this.fpWriteScaled(arg1, 0);
                ...
                break;
            case 203:
                this.fpWriteScaled(arg1, this.fpReadScaled(arg2));
                this.setStatus(0);
                break;
            case 204:
                if (arg1 + 2 >= this.data.length)
                    this.setStatus(16);
                else {
                    this.data[arg1 + 2] = this.fpReadScaled(arg1) & this.mask();
                    this.setStatus(0);
                }
                break;
            case 205:
            ...
            case 206:
                ...
                break;
            case 207:
                if (this.fpReadScaled(arg1) === 0)
                    result = 0;
                else if (this.fpReadScaled(arg1) < 0)
                    result = this.mask();
                else
                    result = 1;
                break;
            ...
            case 209:
                ...
                break;
            case 210:
                this.fpWriteScaled(arg1, this.fpReadScaled(arg2) + this.fpReadScaled(arg0));
                this.setStatus(0);
                break;
            case 211:
                this.fpWriteScaled(arg1, this.fpReadScaled(arg2) - this.fpReadScaled(arg0));
                ...
                break;
            case 212:
                this.fpWriteScaled(arg1, Math.trunc((this.fpReadScaled(arg2) * this.fpReadScaled(arg0)) / this.fpScaleConst()));
                this.setStatus(0);
                break;
            case 213:
                if (this.fpReadScaled(arg0) === 0) {
                    this.setStatus(15);
                }
                else {
                    this.fpWriteScaled(arg1, Math.trunc((this.fpReadScaled(arg2) * this.fpScaleConst()) / this.fpReadScaled(arg0)));
                    this.setStatus(0);
                }
                break;
            case 214:
                if (this.fpReadScaled(arg2) === this.fpReadScaled(arg0))
                    result = 0;
                else if (this.fpReadScaled(arg2) > this.fpReadScaled(arg0))
                    result = 1;
                else
                    result = this.mask();
                break;
            case 215:
                this.fpWriteScaled(arg1, Math.abs(this.fpReadScaled(arg2)));
                this.setStatus(0);
                break;
            case 216:
                this.fpWriteScaled(arg1, -this.fpReadScaled(arg2));
                ...
                break;
            case 217:
                this.fpWriteScaled(arg1, this.fpReadScaled(arg1));
                this.setStatus(0);
                break;
            case 218:
                this.fpWriteScaled(arg1, this.fpReadScaled(arg1));
                ...
                break;
            case 219:
                this.fpWriteScaled(arg1, Math.trunc(this.fpReadScaled(arg1) / this.fpScaleConst()) * this.fpScaleConst());
                this.setStatus(0);
                break;
            case 220:
                this.fpWriteScaled(arg1, this.toSignedCell(arg2) * this.fpScaleConst());
                this.setStatus(0);
                break;
            case 221:
                this.fpWriteScaled(arg1, Math.trunc(Number(this.readDataString(arg2)) * this.fpScaleConst()));
                this.setStatus(0);
                break;
            case 222:
                this.writeDataString(arg2, String(this.fpReadScaled(arg1) / this.fpScaleConst()));
                this.setStatus(0);
                break;
            case 223:
                this.output += String(this.fpReadScaled(arg2) / this.fpScaleConst());
                this.setStatus(0);
                break;
            case 224:
                this.fpWriteScaled(arg1, this.fpReadScaled(arg1) * Math.trunc(Math.pow(10, arg2)));
                this.setStatus(0);
                break;
            case 230:
            case 231:
            ...
            case 240:
                {
                    ...
                    this.data[arg1 + 0] = 80;
                    this.data[arg1 + 1] = 1;
                    this.data[arg1 + 3] = this.data[arg2 + 3] ?? 0;
                    if (deg <= 0) {
                        this.data[arg1 + 2] = 0;
                        this.data[arg1 + 4] = 0;
                        this.setStatus(0);
                    ...
                    for (let i = 1; i <= deg; i++)
                        this.data[arg1 + 3 + i] = (this.toSignedCell(this.data[arg2 + 4 + i] ?? 0) * i) & this.mask();
                    this.setStatus(0);
                }
                break;
            case 241:
                {
                    ...
                    for (let i = 0; i <= deg; i++)
                        this.data[arg1 + 5 + i] = Math.trunc(this.toSignedCell(this.data[arg2 + 4 + i] ?? 0) / (i + 1)) & this.mask();
                    this.setStatus(0);
                }
                break;
            case 242:
                {
                    ...
                    let acc = 0;
                    for (let i = deg; i >= 0; i--)
                        acc = acc * this.toSignedCell(arg2) + this.toSignedCell(this.data[arg1 + 4 + i] ?? 0);
                    result = acc;
                }
                break;
            case 243:
            ...
            case 254:
                ...
                break;
            default:
                this.markUnsupportedMeta(id, "meta servis tanimli degil veya internal yorumlayicida henuz uygulanmadi");
                break;
        }
        if (result !== undefined) {
    ...
    markUnsupportedMeta(id, reason) {
        this.setStatus(5);
        if (this.unsupportedMetaReported.has(id)) {
            return;
        }
        this.diagnostics.push((0, metaRuntime_1.buildUnsupportedMetaMessage)(id, reason));
    ...
    toSignedCell(v) {
        ...
        if (this.cellBits === 8 && (masked & 0x80))
            return masked - 0x100;
        if (this.cellBits === 16 && (masked & 0x8000))
            return masked - 0x10000;
        if (this.cellBits === 32 && (masked & 0x80000000))
    ...
    matCellIndex(base, r, c) {
        if (!this.matValid(base))
            return -1;
        const rows = this.data[base + 5] | 0;
        const cols = this.data[base + 6] | 0;
    ...
    matInit(base, rows, cols, typ, scale) {
        if (base < 0 || rows <= 0 || cols <= 0)
            return false;
        const total = rows * cols;
        if (base + 16 + total - 1 >= this.data.length)
            return false;
    ...
    fpScaleConst() { return 1000000; }
    fpReadScaled(base) { return this.toSignedCell(this.data[base] ?? 0); }
    fpWriteScaled(base, v) { if (base >= 0 && base < this.data.length)
        this.data[base] = v & this.mask(); }
    readDataString(start) {
        let s = "";
    ...
    writeDataString(start, s) {
        for (let i = 0; i < s.length && start + i < this.data.length; i++)
            this.data[start + i] = s.charCodeAt(i) & this.mask();
        if (start + s.length < this.data.length)
            this.data[start + s.length] = 0;
    }
    exprEvalRpn(exprBase, x) {
        if (exprBase < 0 || exprBase + 4 >= this.data.length)
            return 0;
        if ((this.data[exprBase] ?? 0) !== 69)
            return 0;
        ...
        while (ip < exprBase + 4 + count && ip < this.data.length) {
            ...
            if (tok === 1) {
                st.push(this.toSignedCell(this.data[ip++] ?? 0));
                continue;
            }
            if (tok === 2) {
                st.push(x);
            ...
            if (tok === 10 || tok === 11 || tok === 12 || tok === 13 || tok === 14) {
                ...
                if (tok === 10)
                    ...
                    st.push(Math.trunc(Math.pow(a, b)));
                continue;
            }
            if (tok === 20 || tok === 21 || tok === 22 || tok === 23 || tok === 24 || tok === 25 || tok === 30 || tok === 31) {
                ...
                const a = st.pop();
                if (tok === 20)
                    st.push(Math.trunc(Math.sin(a)));
                else if (tok === 21)
                    st.push(Math.trunc(Math.cos(a)));
                else if (tok === 22)
                    st.push(Math.trunc(Math.tan(a)));
                else if (tok === 23)
                    st.push(Math.trunc(Math.exp(a)));
                else if (tok === 24)
                    st.push(a <= 0 ? 0 : Math.trunc(Math.log(a)));
                else if (tok === 25)
                    st.push(a < 0 ? 0 : Math.trunc(Math.sqrt(a)));
                else if (tok === 30)
                    st.push(-a);
                else
                    st.push(Math.abs(a));
    ...
    window(mem, start, count) {
        return (0, trace_1.collectWindow)(mem, start, count, traceReader_1.ascii);
    }
    nonZero(mem, limit) {
        return (0, trace_1.collectNonZero)(mem, limit, traceReader_1.ascii);
    }
    ...
    mask() { return (0, memory_1.cellMaskForBits)(this.cellBits); }
    rngSeed(seed) { this.rngState = seed >>> 0; }
    rngNext() { this.rngState = (Math.imul(1664525, this.rngState) + 1013904223) >>> 0; return this.rngState / 4294967296; }
    signBit() { return (0, memory_1.signBitForBits)(this.cellBits); }
    scale() { return (0, memory_1.scaleForBits)(this.cellBits); }
